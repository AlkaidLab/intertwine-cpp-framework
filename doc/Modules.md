...
连接、发送、接收和状态查询的公开接口见 [API.md](API.md#客户端传输)。
...
服务端接口的生命周期方法见 [API.md](API.md#servertransport)。这里仅说明实现选择：它将路由层与具体监听器解耦。
...
`TransferParams`、`TransferStats` 与 `IFileTransfer` 的接口定义见 [API.md](API.md#文件传输)。业务层负责路径授权和 Range 校验；策略只负责发送生命周期。
